// Ported from MKS (KolonyTools.AC), GPL. Source: refs/mks/AC/CustomAstronautComplexUISpawner.cs
using KSP.UI;
using UnityEngine;
...
namespace CrewHiring.AC
{
    class CustomAstronautComplexUISpawner : MonoBehaviour
    {
        private void Start()
        {
            ...
            if (uiCam == null)
            {
                Debug.LogError("[CrewHiring] UIMainCamera not found");
                return;
            }
            ...
            if (camera == null)
            {
                Debug.LogError("[CrewHiring] Camera attached to UIMainCamera not found");
                return;
            }
