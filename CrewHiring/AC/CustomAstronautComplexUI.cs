...
// Ported from MKS (KolonyTools.AC), GPL. Source: refs/mks/AC/CustomAstronautComplexUI.cs
...
namespace CrewHiring.AC
{
    class CustomAstronautComplexUI : MonoBehaviour
    {
        ...
        // 已知上游（MKS）继承瑕疵：以下 static 状态跨 AC 开关残留旧值，1:1 移植保留原行为
        ...
        public void Initialize(Rect guiRect)
        {
            ...
            if (!HireOptions.CustomKerbonautsEnabled)
            {
                KStupidity = 50;
                KCourage = 50;
            }
        ...
        private static ProtoCrewMember SpawnKerbal(ProtoCrewMember.Gender? chosenGender)
        {
            ...
            Debug.LogError("[CrewHiring] Failed spawning a kerbal with the wanted gender");
        ...
        private int costMath()
        {
            ...
            if (HireOptions.CostCapEnabled)
                defaultCost = Math.Min(defaultCost, HireOptions.GetMaxCost);

            if (_kolonists[KCareer].isBase && !HireOptions.AlternateCoreCostEnabled)
                return defaultCost;
            if (!_kolonists[KCareer].isBase && !HireOptions.AlternateKolonistCostEnabled)
                return defaultCost;

            dCheck();
            float fearlessCost = 0;
            float basecost = HireOptions.GetKolonistCost;
            if (_kolonists[KCareer].isBase)
                basecost = HireOptions.GetCoreCost;
            ...
            if (HireOptions.CostCapEnabled)
                finalcost = Math.Min(finalcost, HireOptions.GetMaxCost);
        ...
        private void OnGUI()
        {
            ...
            GUILayout.BeginArea(_areaRect);
            {
                ...
                if (HireOptions.KolonistHiringEnabled)
                {
